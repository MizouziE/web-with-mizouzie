...
```sh
docker run --detach --name mariadb-container --env MARIADB_USER=mizouzie --env MARIADB_PASSWORD=mizouzie_loves_mariadb --env MARIADB_ROOT_PASSWORD=root -p 3306 mariadb:latest
```
...
- Use `docker ps` to output a list of any running containers with the most important details
...
Also as a lot of applications are deployed to production using docker, having a local development repository with as close to production environment variables is always a plus.
